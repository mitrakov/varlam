 ...
 */
@WebServlet(urlPatterns = Array("/category/list"))
class VarlamListCategory extends VarlamServlet {
  override def doGet(req: HttpServletRequest, resp: HttpServletResponse): Unit = authenticated(req, resp) {
    ...
    def f(category: Category): JSONObject = JSONObject(Map(
      ...
      "items" -> JSONArray(category.getCategories.toArray(Array.empty[Category]).toList map f)
    ))
